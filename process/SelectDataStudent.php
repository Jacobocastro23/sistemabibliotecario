...
if(mysqli_num_rows($selectStudent)>=1){
    ...
    <div class="group-material">
        ...
        <select class="material-control tooltips-general" name="studentSection" data-toggle="tooltip" data-placement="top" title="Elige la sección a la que pertenece el alumno">';
            $AllSectionsSelec=ejecutarSQL::consultar("SELECT * FROM seccion WHERE CodigoSeccion <> '".$dataStudent['CodigoSeccion']."' ORDER BY Nombre ASC");
            ...
            echo '<option value="'.$dataSts['CodigoSeccion'].'">'.$dataSts['Nombre'].' (Actual)</option>';
            while($fila=mysqli_fetch_array($AllSectionsSelec, MYSQLI_ASSOC)){
            ...
            mysqli_free_result($AllSectionsSelec);
