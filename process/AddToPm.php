<?php
	...
	if(isset($_POST['addBookMethod'])){
		$SBC=ejecutarSQL::consultar("SELECT CodigoLibro FROM libro WHERE CodigoLibroManual='$bookCode'");
		if(mysqli_num_rows($SBC)>=1){
			$dataSBC=mysqli_fetch_array($SBC, MYSQLI_ASSOC);
			$bookCode=$dataSBC['CodigoLibro'];
		}
		mysqli_free_result($SBC);
		$startDate=$currentDateForm;
		$endDate=$currentDateForm;  
		$CantL=1;
	}else{
		...
		if(empty($_SESSION['prestmultiple'][$bookCode])){
			...
			if(mysqli_num_rows($CBS)>=1){
				...
				echo '<script type="text/javascript">
				    swal({ 
				        title:"¡Ocurrió un error inesperado!", 
				        text:"No hay ningún libro registrado con el código ingresado", 
				        type: "error", 
				        confirmButtonText: "Aceptar" 
				    });
				</script>';
